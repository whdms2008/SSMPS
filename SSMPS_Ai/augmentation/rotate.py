...
if __name__ == "__main__":
    fileEx = r'.jpg'
    list = [file for file in os.listdir(path.image_path)
            if file.endswith(fileEx)]
    rotate = [30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]
        
    for ro in rotate:
        for image_name in list:

            # Load image
            img = cv2.imread(f"{path.image_path}/{image_name}", cv2.IMREAD_COLOR)

            image_path = f"{path.image_path}/{image_name}"
            angle = ro
            ...
            # initiate the class
            im = yoloRotatebbox(image_name, image_ext, ro)

            bbox = im.rotateYolobbox()
            image = im.rotate_image()

            # to write rotateed image to disk
            cv2.imwrite(image_name+'_rotated' + str(angle) + '.jpg', image)

            file_name = image_name + '_rotated' + str(angle) + '.txt'
